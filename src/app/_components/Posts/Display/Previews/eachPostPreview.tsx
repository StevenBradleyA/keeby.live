...
import EachPostPreviewImage from "./eachPostPreviewImage";
import EachPostPreviewLike from "./eachPostPreviewLike";
import { getServerAuthSession } from "~/server/auth";
import EachPostPreviewFavorite from "./eachPostPreviewFavorite";
import Image from "next/image";
...
export default async function EachPostPreview({ post }: EachPostPreviewProps) {
    ...
    return (
        <div className="w-96 h-72 bg-black/30 rounded-xl flex flex-col p-2 text-sm items-start text-mediumGray">
            <EachPostPreviewImage post={post} />
            <h1 className="mt-2 w-full flex no-wrap overflow-hidden whitespace-nowrap text-base text-white">
                {post.title}
            </h1>
            <div className="flex w-full justify-between mt-1">
                <h2 className="bg-white/20 py-1 px-4 rounded-2xl text-green-500">
                    {post.tag}
                </h2>
                <div className="gap-5 flex">
                    <EachPostPreviewFavorite post={post} session={session} />
                    <EachPostPreviewLike post={post} session={session} />
                </div>
            </div>
